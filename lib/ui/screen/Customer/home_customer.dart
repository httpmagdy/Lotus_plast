        ...
        builder: (controller){
          ...
          return controller.homeLoading.value
            ...
            children: <Widget>[
              ...
              Center(
                child: [
                  controller.allPreview.length == 0 ?NoSomethingYet(title: 'لم تقم بأضافه معاينات بعد !')  :ListView.separated(
                    physics: NeverScrollableScrollPhysics(),
                    shrinkWrap: true,
                    itemBuilder: (context, index) => CardCustomer(
                      onTap: () {

                        if(controller.allPreview[index].statusActive == 0){
                          customSnackBar(title: "قيد التقديم", body: "لا يمكنك فتح هذه المعاينه");
                        }else{
                          Get.to(DetailsPreview(
                              controller.allPreview[index]));
                        }
                      },
                  ...
                  controller.allDamage.length == 0 ?  NoSomethingYet(title: 'لم تقم بأضافه اعطال بعد!'): ListView.separated(
                    physics: NeverScrollableScrollPhysics(),
                    shrinkWrap: true,
                    itemBuilder: (context, index) => CardCustomerMaintenance(
                      onTap: () {
                        Get.to(DetailsMaintenance(controller.allDamage[index]));
                      },
